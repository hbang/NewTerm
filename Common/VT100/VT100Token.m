...
+ (instancetype)tokenForControlCharacter:(unsigned char)controlCharacter {
	VT100Token *token = [[self alloc] init];
...
- (void)dealloc {
	...
	if (_asciiData.screenChars && _asciiData.screenChars->buffer != _asciiData.screenChars->staticBuffer) {
...
- (BOOL)isAscii {
	return _type == VT100_ASCIISTRING;
}

- (BOOL)isStringType {
	return (_type == VT100_STRING || _type == VT100_ASCIISTRING);
}
...
- (NSString *)stringForAsciiData {
	return [[NSString alloc] initWithBytes:_asciiData.buffer length:_asciiData.length encoding:NSASCIIStringEncoding];
}
