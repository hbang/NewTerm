...
- (void)initScreenWithWidth:(int)width Height:(int)height {
		...
		HBLogDebug(@"-[VT100Screen initScreenWithWidth:%d Height:%d]", width, height );
#endif
...
{
#if DEBUG_METHOD_TRACE
		HBLogDebug(@"-[VT100Screen setWidth:%d height:%d]",
						width, height);
#endif
...
#if DEBUG_METHOD_TRACE
	HBLogDebug(@"-[VT100Screen resizeWidth:%d height:%d]",
		  width, height);
#endif
...
- (void)putToken:(VT100Token *)token {
	 ...
	 default:
												HBLogDebug(@" Unexpected token.type = %d",
																token.type);
												break;
		}
...
- (void)clearBuffer {
#if DEBUG_METHOD_TRACE
		HBLogDebug(@"-[VT100Screen clearBuffer]" );
#endif
...
#if DEBUG_METHOD_TRACE
		HBLogDebug(@"-[VT100Screen clearScrollbackBuffer]" );
#endif
...
- (void)setString:(NSString *)string ascii:(BOOL)ascii {
		...
		if (ascii) {
			...
			free(sc);
				...
				// HBLogDebug(@"%d(%d):%@",[_terminal charset],_charset[[_terminal charset]],string);
				free(sc);
		} else {
...
#if DEBUG_METHOD_TRACE
		HBLogDebug(@"setString done at %d", _cursorX);
#endif
}
...
#if DEBUG_METHOD_TRACE
		HBLogDebug(@"-[VT100Screen setNewLine](%d,%d)-[%d,%d]", _cursorX, _cursorY, _scrollTop, _scrollBottom);
#endif
...
#if DEBUG_METHOD_TRACE
		HBLogDebug(@"-[VT100Screen deleteCharacter]: %d", n);
#endif
...
{
#if DEBUG_METHOD_TRACE
		HBLogDebug(@"-[VT100Screen backSpace]");
#endif
		if (_cursorX > 0)
				_cursorX--;
}
...
#if DEBUG_METHOD_TRACE
		HBLogDebug(@"-[VT100Screen clearScreen]; _cursorY = %d", _cursorY);
#endif
		[self acquireLock];
...
#if DEBUG_METHOD_TRACE
		HBLogDebug(@"-[VT100Screen cursorLeft:%d]",
						n);
#endif
		if (x < 0)
				x = 0;
...
#if DEBUG_METHOD_TRACE
		HBLogDebug(@"-[VT100Screen cursorRight:%d]",
						n);
#endif
		if (x >= _width)
				x =	 _width - 1;
...
#if DEBUG_METHOD_TRACE
		HBLogDebug(@"-[VT100Screen cursorUp:%d]",
						n);
#endif
		if(_cursorY >= _scrollTop)
...
#if DEBUG_METHOD_TRACE
		HBLogDebug(@"-[VT100Screen cursorDown:%d, Y = %d; _scrollBottom = %d]",
...
#if DEBUG_METHOD_TRACE
		HBLogDebug(@"-[VT100Screen cursorToX:%d]",
						x);
#endif
		x_pos = (x-1);
...
#if DEBUG_METHOD_TRACE
		HBLogDebug(@"-[VT100Screen cursorToX:%d Y:%d]",
						x, y);
#endif
		int x_pos, y_pos;
...
- (void)saveCursorPosition {
		int i;
#if DEBUG_METHOD_TRACE
		HBLogDebug(@"-[VT100Screen saveCursorPosition]");
#endif
...
- (void)restoreCursorPosition {
		int i;
#if DEBUG_METHOD_TRACE
		HBLogDebug(@"-[VT100Screen restoreCursorPosition]");
#endif
...
#if DEBUG_METHOD_TRACE
		HBLogDebug(@"-[VT100Screen scrollDown]");
#endif
...
#if DEBUG_METHOD_TRACE
		HBLogDebug(@"-[VT100Screen insertBlank; %d]", n);
#endif

		// HBLogDebug(@"insertBlank[%d@(%d,%d)]",n,_cursorX,_cursorY);
...
#if DEBUG_METHOD_TRACE
		HBLogDebug(@"-[VT100Screen insertLines; %d]", n);
#endif
		// HBLogDebug(@"insertLines %d[%d,%d]",n, _cursorX,_cursorY);
...
#if DEBUG_METHOD_TRACE
		HBLogDebug(@"-[VT100Screen deleteLines; %d]", n);
#endif
